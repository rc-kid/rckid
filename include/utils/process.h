...
namespace utils {
    ...
    public:
        ...
        Command(std::string_view cmd, std::initializer_list<std::string_view> args, std::string_view wd):
            cmd_{cmd},
            args_{str::vector(args)},
            wd_{wd} {
        }
        ...
        std::string & workingDirectory() { return wd_; }

        Command & setCommand(std::string_view cmd) {
            cmd_ = cmd;
            return *this;
        }

        Command & setWorkingDirectory(std::string_view wd) {
            wd_ = wd;
            return *this;
        }

        Command & addArg(std::string_view arg) {
