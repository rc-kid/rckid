...
public:
    TestApp() {
        ...
        ap_->animate()
            << MoveHorizontally(ap_, 0, 240 - 20)->setRepeat(true)->setEasingFunction(easing::mirror)->setDelayMs(3000);
        ap_->animate()
            << MoveVertically(ap_, 0, 160 - 20)->setRepeat(true)->setEasingFunction(easing::mirror)->setDelayMs(2000);
