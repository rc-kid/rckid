...
namespace rckid {
    ...
    protected:
        ...
        void update() override {
            ...
            // check ball position
            if (ball_.y() < 5) {
                ball_.setY(5 + (5 - ball_.y()));
                deltaBall_.setY(deltaBall_.y() * -1); 
            }
            if (ball_.y() >= 235) {
                ball_.setY( 470 - ball_.y());
                deltaBall_.setY(deltaBall_.y() * -1); 
            }
            // check the collision with paddles
            if (!fail_) {
                if (ball_.x() >= 300) {
                    ball_.setX( 600 - ball_.x());
                    deltaBall_.setX(deltaBall_.x() * -1);
                    setRumbler(RumblerEffect::Nudge());
                    // TODO pong
                }
                if (ball_.x() <= 20) {
                    if (ball_.y().inRange(left_ - 20, left_ + 20)) {
                        ball_.setX( 40 - ball_.x());
                        deltaBall_.setX(deltaBall_.x() * -1);
                        setRumbler(RumblerEffect::Nudge());
                        // TODO pong
                    } else {
        ...
        void draw() {
            ...
            for (int i = 10; i < 240; i += 20)
                driver_.fill(Rect::XYWH(159, i, 2, 10), Color::DarkGray());
            // draw the player score
            drawScore(scoreLeft_, 120);
            drawScore(scoreRight_, 200 - 20);
            // draw player paddles
            driver_.fill(Rect::XYWH(5, left_ - 20, 10, 40), Color::White());
            driver_.fill(Rect::XYWH(305, right_ - 20, 10, 40), Color::White());
            // draw the ball
            driver_.fill(Rect::XYWH(ball_ - Point{5,5}, 10, 10), Color::White());
        }
        ...
        void drawScore(unsigned score, int fromX) {
            ...
            for (int x = 0; x < 4; ++x) {
                ...
                for (int y = 0; y < 8; ++y) {
                    if (col & (1 << (7 - y)))
                        driver_.fill(Rect::XYWH(fromX + (3 - x) * 5, 10 + y * 5, 5, 5), Color::Gray());
                }
            }
        }
